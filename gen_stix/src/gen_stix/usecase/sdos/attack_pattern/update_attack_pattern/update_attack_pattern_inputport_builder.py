...
@dataclass
class UpdateAttackPatternInputPortBuilder:
    """This class defined the function to easily build the input contract

    Attributes:
    -----------
    __input: UpdateAttackPatternInputPort
        the input contract
    ...
    def create(self):
        ...
        self.__input = UpdateAttackPatternInputPort()
        return self
            ...
            if not isinstance(external_references, List):
                ...
                for external_ref in external_references:
                    if not isinstance(external_ref, Dict):
                        ...
                        and not external_ref.external_id.startswith("CAPEC-")
                    ):
                        raise ValueError(
                            "`external_references` must be a List[external-reference]."
                        )
    ...
    def build(self) -> UpdateAttackPatternInputPort:
