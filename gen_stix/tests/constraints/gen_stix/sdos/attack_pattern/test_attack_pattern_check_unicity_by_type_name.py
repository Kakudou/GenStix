...
@given(
    parsers.parse(
        "Multiple AttackPatterns are created with the same {name} and {type_}."
    ),
...
@then(
    parsers.parse(
        "A NameError exception should be raised with the message 'The AttackPattern you want, already exists'."
    )
)
def then_attack_pattern_check_unicity_by_type_name(context):
    assert (
        context["output_contract"].error
        == "The AttackPattern you want, already exist"
    )
