...
@given(parsers.parse("An AttackPattern is created with a {type_} different from `attack-pattern` and a valid {name}."), target_fixture="context")
...
@then(parsers.parse("A ValueError should be raised, stating that the value of type must be `attack-pattern`."))
