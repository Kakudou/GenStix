...
@given(parsers.parse("An AttackPattern is created without any type but with a {name}."), target_fixture="context")
...
@then(parsers.parse("A ValueError should be raised, stating the requirements for the type field."))
def then_attack_pattern_stix21_required_type(context):
    assert type(context["error"]) is ValueError
    assert str(context["error"]) == "`type` is a required field for AttackPattern"
