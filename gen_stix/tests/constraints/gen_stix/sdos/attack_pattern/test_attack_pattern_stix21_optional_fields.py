...
from gen_stix.src.gen_stix.usecase.\
    sdos.external_reference.create_external_reference.create_external_reference_inputport_builder\
    import CreateExternalReferenceInputPortBuilder
from gen_stix.src.gen_stix.usecase.\
    sdos.kill_chain_phase.create_kill_chain_phase.create_kill_chain_phase_inputport_builder\
    import CreateKillChainPhaseInputPortBuilder
...
@given(parsers.parse("An AttackPattern is created with {external_references}, {description}, {aliases}, {kill_chain_phases}, a {name}, and a {type_}."), target_fixture="context")
def given_attack_pattern_stix21_optional_fields(external_references, description, aliases, kill_chain_phases, name, type_):
    list_eref = []
    for external_ref in json.loads(external_references):
        eref_contract = CreateExternalReferenceInputPortBuilder()\
            .create()\
            .with_source_name(external_ref["source_name"])\
            .with_description(external_ref["description"])\
            .build()
        eref_usecase = Container.get_usecase_repo("CreateExternalReference", STORAGE_ENGINE)
        eref_contract = eref_usecase.execute(eref_contract)
        if eref_contract.error is None:
            list_eref.append({"source_name": eref_contract.source_name, "description": eref_contract.description, "external_id": eref_contract.external_id})

    list_kchain = []
    for kill_chain_phase in json.loads(kill_chain_phases):
        kchain_contract = CreateKillChainPhaseInputPortBuilder()\
            .create()\
            .with_kill_chain_name(kill_chain_phase["kill_chain_name"])\
            .with_phase_name(kill_chain_phase["phase_name"])\
            .build()
        kchain_usecase = Container.get_usecase_repo("CreateKillChainPhase", STORAGE_ENGINE)
        kchain_contract = kchain_usecase.execute(kchain_contract)
        if kchain_contract.error is None:
            list_kchain.append({"kill_chain_name": kchain_contract.kill_chain_name, "phase_name": kchain_contract.phase_name})
...
@then(parsers.parse("The fields {external_references}, {description}, {aliases}, {kill_chain_phases}, {name}, and {type_} should match the input values."))
def then_attack_pattern_stix21_optional_fields(context, external_references, description, aliases, kill_chain_phases, name, type_):
    assert context["output_contract"]\
        .external_references == context["list_eref"]
    assert context["output_contract"]\
        .description == description
    assert context["output_contract"]\
        .aliases == json.loads(aliases)
    assert context["output_contract"]\
        .kill_chain_phases == context["list_kchain"]
    assert context["output_contract"]\
        .name == name
    assert context["output_contract"]\
        .type_ == type_
