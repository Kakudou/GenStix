...
@given(
    parsers.parse(
        "An AttackPattern is created with a {type_}, {name}, and invalid {external_references}."
    ),
    target_fixture="context",
)
def given_attack_pattern_stix21_constraint_external_references(
...
@then(
    parsers.parse(
        "A ValueError should be raised, stating that {external_references} should be a list of external-references."
    )
)
def then_attack_pattern_stix21_constraint_external_references(
