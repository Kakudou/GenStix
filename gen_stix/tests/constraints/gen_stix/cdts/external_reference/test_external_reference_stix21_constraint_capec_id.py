...
@given(parsers.parse("An ExternalReference is created with {source_name} set to \"capec\" and an invalid {external_id}."), target_fixture="context")
def given_external_reference_stix21_constraint_capec_id(source_name, external_id):
...
def when_external_reference_stix21_constraint_capec_id(context, ):
    try:
...
@then(parsers.parse("A ValueError should be raised, stating that {external_id} must be formatted as CAPEC-[id]."))
def then_external_reference_stix21_constraint_capec_id(context, external_id):
    assert type(context["error"]) is ValueError
    assert str(context["error"]) == "`external_id` with a `source_name` 'capec' must be a formatted as CAPEC-[id]." or str(context["error"]) == "No external-reference CAPEC found for None."
