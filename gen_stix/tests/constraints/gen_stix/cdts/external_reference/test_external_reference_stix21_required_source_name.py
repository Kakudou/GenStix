...
@given(
    parsers.parse("An ExternalReference is created without a {source_name}."),
...
def when_external_reference_stix21_required_source_name(
    context,
):
    try:
...
@then(
    parsers.parse(
        "A ValueError should be raised, stating that {source_name} is required."
    )
)
