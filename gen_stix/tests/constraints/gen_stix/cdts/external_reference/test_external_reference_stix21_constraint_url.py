...
@given(
    parsers.parse(
        "An ExternalReference is created with {source_name} and an invalid {url}."
    ),
...
def given_external_reference_stix21_constraint_url(source_name, url):
    ...
    return {"url": url, "input_contract": input_contract}


@when(parsers.parse("The ExternalReference is created."))
def when_external_reference_stix21_constraint_url(
    context,
):
    url = {}
    try:
...
@then(
    parsers.parse(
        "A ValueError should be raised, stating that {url} must conform to RFC3986."
    )
)
