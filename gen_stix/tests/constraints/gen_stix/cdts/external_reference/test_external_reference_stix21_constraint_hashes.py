...
@given(parsers.parse("An ExternalReference is created with {source_name} and invalid {hashes}."), target_fixture="context")
...
def when_external_reference_stix21_constraint_hashes(context, ):
    hashes = ""
    try:
...
@then(parsers.parse("A ValueError should be raised, stating that {hashes} must be a valid dictionary of hashes with SHA-256 included when possible."))
def then_external_reference_stix21_constraint_hashes(context, hashes):
    assert type(context["error"]) is ValueError
    assert str(context["error"]) == "`hashes` must be a Dict." \
