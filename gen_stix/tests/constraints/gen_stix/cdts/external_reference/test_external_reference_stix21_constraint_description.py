...
@given(parsers.parse("An ExternalReference is created with {source_name} and invalid {description}."), target_fixture="context")
def given_external_reference_stix21_constraint_description(source_name, description):
...
def when_external_reference_stix21_constraint_description(context, ):
    description = {}
    try:
...
@then(parsers.parse("A TypeError should be raised, stating that {description} must be a string."))
def then_external_reference_stix21_constraint_description(context, description):
