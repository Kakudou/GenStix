...
@given(parsers.parse("An ExternalReference is created with {source_name} and an invalid {external_id}."), target_fixture="context")
def given_external_reference_stix21_constraint_external_id(source_name, external_id):
...
def when_external_reference_stix21_constraint_external_id(context, ):
    external_id = {}
    try:
...
@then(parsers.parse("A TypeError should be raised, stating that {external_id} must be a string."))
def then_external_reference_stix21_constraint_external_id(context, external_id):
