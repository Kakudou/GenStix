...
@given(parsers.parse("An ExternalReference is created with an invalid {source_name}."), target_fixture="context")
...
def when_external_reference_stix21_constraint_source_name(context, ):
    source_name = {}
    try:
...
@then(parsers.parse("A TypeError should be raised, stating that {source_name} must be a string."))
def then_external_reference_stix21_constraint_source_name(context, source_name):
