...
@given(
    parsers.parse(
        "An ExternalReference is created with {source_name}, {description}, {url}, {hashes}, and {external_id}."
    ),
...
def when_external_reference_stix21_optional_fields(
    context,
):
...
@then(
    parsers.parse(
        "The fields {description}, {url}, {hashes}, and {external_id} of the object should be as provided during creation."
    )
)
def then_external_reference_stix21_optional_fields(
    context, description, url, hashes, external_id
):
    assert context["output_contract"].description == description
    assert context["output_contract"].url == url
    assert context["output_contract"].hashes == json.loads(hashes)
    assert context["output_contract"].external_id == context["external_id"]
