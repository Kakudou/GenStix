...
from gen_stix.src.gen_stix.usecase.cdts.external_reference.delete_external_reference.delete_external_reference_inputport_builder import (
...
@given(
    parsers.parse(
        "An ExternalReference is created with {source_name}, {description}."
    ),
...
@when(
    parsers.parse(
        "The ExternalReference is created, updated with {new_description} and deleted."
    )
)
def when_external_reference_crudl(context, new_description):
    ...
    if "Updated" in new_description:
        ...
        usecase = Container.get_usecase_repo(
            "DeleteExternalReference", STORAGE_ENGINE
        )
...
@then(
    parsers.parse(
        "An ExternalReference should exist with {new_description}, and {source_name}, and one ExternalReference should be deleted."
    )
)
def then_external_reference_crudl(context, new_description, source_name):
    if "Deleted" not in new_description:
        assert context["output_contract"].source_name == source_name
